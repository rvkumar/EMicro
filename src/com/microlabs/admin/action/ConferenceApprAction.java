...
public class ConferenceApprAction extends DispatchAction{
	...
	public ActionForward updateRoomDetails(ActionMapping mapping,
	  ...
	  String updateFloor="update ConferanceRoom_List set status='"+masterForm.getStatus()+"' where LocId='"+masterForm.getLocationId()+"' and "
	...
	public ActionForward editRoomDetails(ActionMapping mapping,
		...
		String getRoomDetails="select loc.LOCID,conf.Floor,conf.Room_Name,conf.Status from ConferanceRoom_List conf,"
	...
	public ActionForward saveFloor(ActionMapping mapping,
	   ...
	   if(saveType.equals("Floor")){
		   ...
		   int check=0;
		  String checkFloor="select count(*) from ConferanceFloor_List where LocId='"+masterForm.getLocationId1()+"' and Floor='"+masterForm.getFloor1()+"'";
		  ...
		  if(check==0){ 
		  String saveFloor="insert into ConferanceFloor_List(LocId,Floor) values('"+masterForm.getLocationId1()+"','"+masterForm.getFloor1()+"')";
		  ...
		  if(i>0){
		   ...
		   int check=0;
			  String checkFloor="select count(*) from ConferanceRoom_List where LocId='"+masterForm.getLocationId()+"' and "
			  + "Floor='"+masterForm.getFloor()+"' and Room_Name='"+masterForm.getRoomName()+"'";
			  ResultSet rsCheck=ad.selectQuery(checkFloor);
			  try{
				 while(rsCheck.next()){
			  ...
			  if(check==0){ 
				  String saveFloor="insert into ConferanceRoom_List(LocId,Floor,Room_Name,status) values('"+masterForm.getLocationId()+"',"
	...
	public ActionForward manageConfroom(ActionMapping mapping,
		...
		EssDao ad=new EssDao();
		String conf="Select * From ConferanceRoom_List ";
		ResultSet rs=ad.selectQuery(conf);
		ArrayList conflist=new ArrayList();
	try {
		while(rs.next())
		{
			ConferenceApprForm data=new ConferenceApprForm();
	...
	public ActionForward deleteApprs(ActionMapping mapping,
        ...
        String LocId="";
	     ...
	     String delete="delete ConferanceRoom_Approvers where LocID='"+LocId+"' and Floor='"+floor+"' and Room='"+room+"'";
	...
	public ActionForward modifyApprs(ActionMapping mapping,
	     ...
	     String loc=masterForm.getLocationId();
	   	...
	   	String delete="delete ConferanceRoom_Approvers where LocID='"+loc+"' and Floor='"+floor+"' and Room='"+room+"'";
	   	int a=ad.SqlExecuteUpdate(delete);
	   	...
	   	if(a>0)
	   	{
			 ...
			 if(!(approver1.equalsIgnoreCase("")))
			  {
				  String saveEssApprovers="insert into ConferanceRoom_Approvers(LocID,Floor,Room,Approver_Id,Priority)" +
	...
	public ActionForward editApprover(ActionMapping mapping,
		...
		ConferenceApprForm masterForm=(ConferenceApprForm)form;
	     ...
	     String getappr="select * from ConferanceRoom_Approvers where LocID='"+LocId+"' and Floor='"+floor+"' and Room='"+room+"' order by Priority ";
			...
			HttpServletRequest request, HttpServletResponse response) 
	{
		...
		ConferenceApprForm masterForm=(ConferenceApprForm)form;
		 ...
		 //chk approvers already present 
			...
			String checkAppr="select count(*) from ConferanceRoom_Approvers  where LocID ='"+loc+"' and Floor='"+floor+"' and Room='"+room+"'";
	...
	public ActionForward newApprover(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		ConferenceApprForm apprForm=(ConferenceApprForm)form;
	...
	public ActionForward getRoomsList(ActionMapping mapping,
		...
		ConferenceApprForm help = (ConferenceApprForm) form;
		EssDao ad=new EssDao();
		...
		try {
			String getFloor="select Room_Name from ConferanceRoom_List where LocId='"+locID+"' and Floor='"+floorID+"'  and status='yes'";
	...
	public ActionForward getFloorList1(ActionMapping mapping,
		...
		try {
			String getFloor="select Distinct(Floor) from ConferanceFloor_List where LocId='"+locID+"' ";
			ResultSet rsFloor=ad.selectQuery(getFloor);
			while(rsFloor.next()){
	...
	public ActionForward getFloorList(ActionMapping mapping,
		...
		ArrayList floorList = new ArrayList();
		try {
			String getFloor="select Distinct(Floor) from ConferanceRoom_List where LocId='"+locID+"' and status='yes'";
			ResultSet rsFloor=ad.selectQuery(getFloor);
			while(rsFloor.next()){
				floorList.add(rsFloor.getString("Floor"));
			}
	...
	public ActionForward approversList(ActionMapping mapping, ActionForm form,
		...
		LinkedList 	appr=new LinkedList();
			String getApprovers="select loc.LOCATION_CODE,conf.Floor,conf.room,count(*) as total from ConferanceRoom_Approvers conf,Location loc "
					+ " where conf.LocID=loc.LOCID group by loc.LOCATION_CODE,conf.Floor,conf.Room";
		ResultSet rs=ad.selectQuery(getApprovers);
		try {
			while(rs.next()){
				ConferenceApprForm masterForm1=new ConferenceApprForm();
