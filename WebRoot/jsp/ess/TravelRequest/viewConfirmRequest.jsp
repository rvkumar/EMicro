...
function confirmrequest(){
	...
	document.forms[0].action="travelrequest.do?method=confirmBookingDetails";
	document.forms[0].submit();
	}
...
function uploadDocument(reqtype,reqno,reqstatus)
{
	...
	var x=reqtype;
	document.forms[0].action="travelrequest.do?method=uploadConfirmDocuments&reqType="+x+"&requstNo="+reqno+"&requststatus="+reqstatus;
	document.forms[0].submit();
}


function deleteDocumentsSelected(reqtype,reqno,reqstatus)
{
	var x=reqtype;
...
if(agree)
{
	...
	document.forms[0].action="travelrequest.do?method=deleteConfirmDocuments&reqType="+x+"&requstNo="+reqno+"&requststatus="+reqstatus;
	document.forms[0].submit();
}
}
}
...
{
	document.forms[0].action="travelrequest.do?method=rejectRecord&requstNo="+reqid;
	document.forms[0].submit();
...
<tr>
				...
				<tr>
					...
					<a href="${abc.filepath}">Passport</a>  
					</td>
					<td>
					Pdf File
					</td>
					<td colspan="3">
					<a href="${abc.fileFullPath}" target="_blank">Pdf</a>
					</td>
					</tr>
...
<td>${abcp4.remarks}<input name="mremarks" value="${abcp4.remarks}" type="hidden">
		          ...
		          <tr>
	<td> &nbsp;&nbsp;&nbsp;&nbsp; Comments<font color="red" size="3">*</font></td>
	<td><textarea rows=""   name="confirm_Comments"  cols="60"></textarea>   </td>
	</tr>
							...
							<div style="margin-left: 300px">
									...
									<logic:equal value="" property="confirmstatus" name="abc">
							  <html:button property="method" styleClass="rounded" value="Accept Booking" onclick="confirmrequest()" style="align:right;width:100px;"/>
							 <%--  <html:button property="method" styleClass="rounded" style="width: 80px"  value="Reject" onclick="cancelrecord(${abc.requestNumber})"></html:button>  --%>
						</logic:equal>
    ...
    <tr><th>Approver Name</th><th>Designation</th><th>Status</th><th>Date</th><th>Comments</th></tr> 
   	<logic:iterate id="abc1" name="appList">
	<tr>
	<td>${abc1.approver }</td>
	<td>${abc1.designation }</td>
	<td>${abc1.approveStatus }</td>
	<td>${abc1.approveDate }</td>
	<td>${abc1.comments }</td>
	...
	<td>${abc.role }</td>
  			...
  			<html:hidden property="requestNumber" /><html:hidden property="reqStatus" />
  			</div>
  			</html:form>			
  </body>
</html>
